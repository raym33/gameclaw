...
It can also be aimed at educational or gamified products, includes a reusable task-based education sim template for huertos, trafico, cocina y otras rutinas con un personaje central, a platformer-focused production template for side-scrollers and precision slices, a dedicated falling-block puzzle template for stacker-style requests, and a fixed-shooter arcade template for “marcianitos” style briefs.
...
- Maze, educational, task-sim, or pressure relic hunt
- Treasure route, precision climb, or combat gauntlet platformer
- Slingshot destruction with Matter physics

The system favors stable playable slices over pretending it can generate any full game from scratch. Today it ships 5 executable runtime families and 13 narrower game type kits on top of them.
...
The repo now also includes reusable runtime presets, scene scaffolds, game type catalogs, and reusable product templates in `src/game/runtimeTemplates.ts`, `src/game/runtimeSceneScaffold.ts`, `shared/gameTypeKits.ts`, `src/game/gameTypeStageLayouts.ts`, `skill/assets/templates/education-gamification.json`, `skill/assets/templates/task-based-education-sim.json`, `skill/assets/templates/platformer-production.json`, `skill/assets/templates/falling-block-puzzle.json`, and `skill/assets/templates/fixed-shooter-arcade.json`, so Codex can reuse genre-level defaults for HUD, timers, player bodies, finish overlays, stage layouts, gameplay tuning, educational gamification loops, task-based educational sims, platformer production direction, falling-block puzzle scoping, and fixed-shooter arcade scoping instead of rediscovering them every time.
...
The skill teaches Codex how to work with this repo, including runtime profiles, executable game type kits, preset kits by genre, reusable scene scaffolds, educational/gamification templates, task-based education sim templates, platformer production templates, falling-block puzzle templates, fixed-shooter arcade templates, asset production workflow, Phaser/Matter validation, sprite fidelity, asset coverage, app-store-style presentation, and the quality bar for movement and physics.
